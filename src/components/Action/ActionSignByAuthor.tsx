...
export const ActionSignByAuthor = <TData extends { id: number, state: DocumentState }>({
  onSignByAuthor,
  data,
  ...props
  ...
  if (data?.state === DocumentState.DRAFT || data?.state === DocumentState.PENDING_AUTHOR_SIGN) {
    return (
      <AlertDialog>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Вы собираетесь подписать этот документ. Убедитесь, что вы ознакомились с его содержимым.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
